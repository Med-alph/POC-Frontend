...
const DoctorPatientRecord = () => {
  ...
  try {
    ...
    const downloadAllSOAPNotes = () => {
        ...
        doc.text(`Age: ${patientData?.age || calculateAge(patientData?.dob)} | Gender: ${patientData?.gender || 'N/A'}`, 14, 37);
        ...
        consultations.forEach((consultation, index) => {
            ...
            // SOAP Notes
            doc.setFont(undefined, 'bold');
            doc.text('Subjective:', 14, yPosition);
            doc.setFont(undefined, 'normal');
            yPosition += 5;
            const subjectiveLines = doc.splitTextToSize(consultation.subjective || 'Not recorded', 170);
            ...
            doc.setFont(undefined, 'normal');
            yPosition += 5;
            const objectiveLines = doc.splitTextToSize(consultation.objective || 'Not recorded', 170);
            ...
            doc.setFont(undefined, 'bold');
            doc.text('Assessment:', 14, yPosition);
            doc.setFont(undefined, 'normal');
            yPosition += 5;
            const assessmentLines = doc.splitTextToSize(consultation.assessment || 'Not recorded', 170);
            ...
            doc.setFont(undefined, 'bold');
            doc.text('Plan:', 14, yPosition);
            doc.setFont(undefined, 'normal');
            yPosition += 5;
            const planLines = doc.splitTextToSize(consultation.plan || 'Not recorded', 170);
            ...
            if (consultation.prescriptions && consultation.prescriptions.length > 0) {
                ...
                consultation.prescriptions.forEach((rx) => {
                    doc.setFont(undefined, 'normal');
                    doc.text(`• ${rx.medicine_name} - ${rx.dosage}, ${rx.frequency}, ${rx.duration}`, 20, yPosition);
                    yPosition += 5;
                });
                yPosition += 5;
            }
            ...
            if (consultation.lab_orders && consultation.lab_orders.length > 0) {
                ...
                consultation.lab_orders.forEach((lab) => {
                    doc.setFont(undefined, 'normal');
                    doc.text(`• ${lab.test_name} - ${lab.instructions || 'No instructions'}`, 20, yPosition);
                    yPosition += 5;
                });
                yPosition += 5;
            }
