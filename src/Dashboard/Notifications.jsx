...
export default function Notifications() {
  ...
  const handleApprove = async () => {
    ...
    if (notifType === 'leave') {
      ...
      const leaveRequestId = modal.notif?.leave_request_id || modal.notif?.notification_leave_request_id;
      if (!leaveRequestId) {
        toast.error("Leave request ID missing.");
        return;
      }
      try {
        setProcessingId(modal.notif.notification_id || modal.notif.id);
      ...
      const notif = modal.notif;
      const metadata = notif?.notification_metadata || notif?.metadata || notif?.data || {};

      const cancellationRequestId = notif?.notification_cancellation_request_id ||
        notif?.cancellation_request_id ||
        metadata?.cancellation_request_id ||
        metadata?.id;

      const appointmentId = notif?.notification_appointment_id ||
      ...
      try {
        setProcessingId(notif.notification_id || notif.id);
          ...
          <div className="space-y-3">
            {notifications.map((notif) => {
              ...
              return (
                <div
                  key={notificationId}
                  className={`bg-white dark:bg-gray-800 border rounded-md transition-colors ${isProcessed || notificationStatus === 'read' || notificationStatus === 'dismissed'
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                      ...
                      <div className="flex-1 space-y-3">
                        <div className="flex items-start gap-3">
                          <div className={`p-2 rounded-md ${isProcessed || notificationStatus === 'read'
                            ...
                            {notifType === 'leave' ? (
                              <FileText className={`h-5 w-5 ${isProcessed
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                              ...
                              {/* Status Badge */}
                              {notificationStatus && (
                                <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded text-xs font-medium ${notificationStatus === 'read' || notificationStatus === 'dismissed'
                      ...
                      <div className="flex items-center sm:items-start gap-2">
                        ...
                        {!isProcessed && notifType === 'cancellation' && (
                          <button
                            ...
                            className={`px-4 py-2 rounded-md font-medium text-sm transition-colors flex items-center gap-2 ${isProcessing
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative z-50 bg-white dark:bg-gray-800 rounded-md shadow-lg w-full max-w-md border border-gray-200 dark:border-gray-700">
              ...
              <div className="p-4 space-y-3">
                <div className={`rounded-md p-3 border ${getNotificationType(modal.notif) === 'leave'
