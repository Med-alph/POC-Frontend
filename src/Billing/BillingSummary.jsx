...
export default function BillingSummary({ subtotal, totalToday, onPayNow, onShowQr, loading, showPayButton = true, patient, hospitalUpiId, isPaymentEnabled }) {
          ...
          <div className="flex-1 w-full lg:w-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 rounded-md p-4 border border-blue-200 dark:border-blue-800">
                <p className="text-xs text-blue-600 dark:text-blue-400 mb-1 font-medium">Net Amount to Pay</p>
          ...
          {showPayButton && (
            <div className="border-t border-gray-100 dark:border-gray-700 pt-6">
              <div className="flex flex-col gap-3">
                ...
                <div className={`grid grid-cols-1 md:grid-cols-${hospitalUpiId ? '4' : '3'} gap-4`}>
                  {/* Digital Payment */}
                  <Button
                    onClick={() => onPayNow('DIGITAL')}
                    disabled={loading || !isPaymentEnabled}
                    className={`${!isPaymentEnabled ? 'bg-gray-100 text-gray-400' : 'bg-blue-600 hover:bg-blue-700 text-white'} h-14 flex flex-col items-center justify-center gap-1 rounded-md`}
                  >
                    <div className="flex items-center gap-2">
                      <CreditCard className="h-4 w-4" />
                      <span className="font-bold">Send to Patient</span>
                    </div>
                    <span className="text-[10px] opacity-80">{isPaymentEnabled ? 'Enable Patient-side Payment' : 'Gateway Disabled'}</span>
                  </Button>
                  ...
                  >
                    <div className="flex items-center gap-2 text-gray-900 dark:text-white">
                      ...
                      <span className="font-bold">Settle Payment</span>
                    </div>
                  ...
                  {hospitalUpiId && (
                    <Button
                      ...
                      disabled={loading}
                      variant="outline"
                      className="border-emerald-200 bg-emerald-50/20 dark:bg-emerald-900/10 h-14 flex flex-col items-center justify-center gap-1 rounded-md hover:bg-emerald-50 dark:hover:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400"
                    >
                      <div className="flex items-center gap-2">
                        <Receipt className="h-4 w-4" />
                        <span className="font-bold">Display UPI QR</span>
                      </div>
                      <span className="text-[10px]">Instant QR Payment</span>
                    </Button>
                  )}
