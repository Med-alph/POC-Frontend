...
export default function DoctorAttendanceCalendar() {
    ...
    return (
        <div className="flex flex-col lg:flex-row gap-6">
            {/* Calendar Card */}
            <Card className="flex-1 border border-gray-200 dark:border-gray-700">
                    ...
                    <div className="w-full flex justify-center">
                        <Calendar
                                    ...
                                    if (record) {
                                        ...
                                        return (
                                            <div className="flex flex-col items-center justify-center mt-1 gap-0.5">
                                                <Icon className={`h-3.5 w-3.5 ${statusColors[record.status]}`} />
                                                <span className={`text-xs font-medium ${statusColors[record.status]}`}>
                            ...
                            tileClassName={({ date, view }) => {
                                ...
                                const baseClasses = "rounded-md transition-colors h-16 flex flex-col items-center justify-center";
                                const todayClass = view === "month" && d === today 
                                    ? "bg-blue-50 dark:bg-blue-900/30 border border-blue-300 dark:border-blue-700 font-semibold" 
                                    : "";
            ...
            {/* Details Panel */}
            <Card className="w-full lg:w-96 border border-gray-200 dark:border-gray-700">
                <CardHeader className="border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
                    <CardTitle className="text-sm font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                        <CalendarIcon className="h-4 w-4 text-gray-500 dark:text-gray-400" />
                        {selectedRecord
                ...
                <CardContent className="p-4 space-y-3">
                    {selectedRecord ? (
                                ...
                                <div className="bg-gray-50 dark:bg-gray-800 p-3 rounded-md border border-gray-200 dark:border-gray-700 space-y-2">
                                    <div className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
                                        ...
                                        <div>
                                            ...
                                            <p className="font-semibold text-sm">{selectedRecord.checkIn}</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
                                        ...
                                        <div>
                                            ...
                                            <p className="font-semibold text-sm">{selectedRecord.checkOut}</p>
                                        </div>
                                    </div>
                                </div>
                            )}
                                ...
                                {selectedRecord.patients.length > 0 ? (
                                    <ul className="space-y-1.5">
                                        {selectedRecord.patients.map((p, idx) => (
                                            <li key={idx} className="flex items-center gap-2 text-xs text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700/50 p-2 rounded-md border border-gray-200 dark:border-gray-600">
                                    ...
                                    <p className="text-xs text-gray-400 dark:text-gray-500">No patients attended</p>
                                )}
                            </div>
                        ...
                        <div className="flex flex-col items-center justify-center py-12 text-center">
                            ...
                            <p className="text-sm text-gray-500 dark:text-gray-400">Click a date to see attendance details</p>
                        </div>
                    )}
                </CardContent>
            </Card>
